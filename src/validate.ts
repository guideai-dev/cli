...
export async function validateCommand(
	...
	if (filesToValidate.length === 0) {
		if (!options.json) {
			console.log(
				chalk.yellow(`No files found for provider: ${options.provider}`)
			)
		}
		return
	}
	...
	if (options.provider && !options.json) {
		console.log(
			chalk.cyan(
				`Filtering for provider "${options.provider}": ${filesToValidate.length} file(s)`
			)
		)
	}
	...
	if (options.json) {
		...
		const totalMessages = results.reduce(
			(sum, r) => sum + r.result.parsedLines,
			0
		)
		const validMessages = results.reduce(
			(sum, r) => sum + r.result.validMessages,
			0
		)
		...
		console.log(
			`Total Messages: ${totalMessages} / Valid: ${chalk.green(validMessages)}`
		)
